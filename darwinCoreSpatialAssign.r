...
#' 		\item	\code{maxCoordUncerPrecision_m}: Maximum value of coordinate uncertainty and precision, in meters.
...
darwinCoreSpatialAssign <- function(
	...
	#########################################################################
		...
		if (length(recs) > 0) {
			...
			maxCoordUncerOrPrecision_m <- pmax(theseCoordPrecision_m, these$coordinateUncertaintyInMeters, na.rm=TRUE)
			
			## distinguish matching/not matching
			doRecsMatch <- stateCountyOfDarwin[recs] %in% stateCountyExtractFromGeog & !is.na(stateExtractFromGeog) & !is.na(countyExtractFromGeog)
			...
			subRecs <- recs[which(these$coordinateUncertaintyInMeters <= minCoordUncerOrPrecision_m & theseCoordPrecision_m < minCoordPrecisionForceCounty_m)]
	...
	#########################################################################
		...
		if (length(recs) > 0) {
			...
			stateExtractFromGeog <- stateCountyExtractFromGeog[ , stateGeogField]
			countyExtractFromGeog <- stateCountyExtractFromGeog[ , countyGeogField]
			stateCountyExtractFromGeog <- tolower(paste(stateExtractFromGeog, countyExtractFromGeog))
			...
			subRecs <- subRecs[which(subRecs %in% recs[doRecsMatch])]
	...
	#########################################################################
		...
		if (length(recs) > 0) {
			...
			## distinguish matching/not matching
			doRecsMatch <- stateFromDarwinCountyFromGeog %in% stateCountyOfGeogLower & !is.na(stateExtractFromGeog) & !is.na(countyExtractFromGeog)
			recsMatch <- recs[doRecsMatch]
			recsNoMatch <- recs[!doRecsMatch]
	...
	#########################################################################
		...
		if (length(recs) > 0) {
			...
			## distinguish matching/not matching
			doRecsMatch <- stateCountyOfDarwin[recs] %in% stateFromGeogCountyFromDarwin & !is.na(stateExtractFromGeog) & !is.na(countyExtractFromGeog)
			recsNoMatch <- recs[!doRecsMatch]
	...
	out$maxCoordUncerPrecision_m <- pmax(out$coordUncer_m, out$coordPrecision_m, na.rm=TRUE)
...
.subCoordPrecision <- function(
	...
	### coordinate precision from coordinates
		...
		numCoordDigits <- pmax(numCoordDigitsLong, numCoordDigitsLat, na.rm=TRUE)
		...
		coordPrecisionDec_m <- enmSdm::coordPrecision(theseSp, dms=FALSE)
		coordPrecisionDms_m <- enmSdm::coordPrecision(theseSp, dms=TRUE)
		
		coordPrecision_m <- pmax(coordPrecisionDec_m, coordPrecisionDms_m)
	...
	### coordinate precision ascertained from verbatim coordinate system
		...
		if (!is.null(coordSystemStringsDegMinSec)) {
			...
			if (length(recs) > 0) {
				...
				coordsPrecisionFromDms <- if (length(recs) == 1) {
					which.max(c(subOut$coordPrecision_m[recs], coordPrecisionFromDms_m[recs])) == 2
				} else {
					omnibus::which.pmax(subOut$coordPrecision_m[recs], coordPrecisionFromDms_m[recs], na.rm=TRUE) == 2
				}
