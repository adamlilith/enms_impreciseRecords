...
### 
### source('C:/Ecology/Drive/Research Active/ENMs - Vaguely Georeferenced Specimen Records/enms_impreciseRecords/01 Asclepias Extent of Occurrence.r')
### source('D:/Ecology/Drive/Research Active/ENMs - Vaguely Georeferenced Specimen Records/enms_impreciseRecords/01 Asclepias Extent of Occurrence.r')
### source('E:/Ecology/Drive/Research Active/ENMs - Vaguely Georeferenced Specimen Records/enms_impreciseRecords/01 Asclepias Extent of Occurrence.r')

### CONTENTS ###
### setup ###
### calculate EOO of accurate vs accurate-plus-inaccurate records ###
### EXAMPLE: plot inaccurate and accurate records ###
...
#############
	...
	library(enmSdm)
	library(legendary)
	# source('./enms_impreciseRecords/Functions for Calculating Range Size.r')
...
say('#####################################################')
say('### EXAMPLE: plot inaccurate and accurate records ###')
say('#####################################################')
	...
	species <- asclepias$meta$usableSpecies$species
		...
		### plot
		dirCreate('./Analysis/')
		png(paste0('./Analysis/Example of Precise & Imprecise Records for  ', thisSpecies, '.png'), width=2 * 1333, height=2 * 833, res=450)
			...
			# plot(thisNam2SpEa, add=TRUE, border='gray', col='gray90')
			plot(thisNam1SpEa, add=TRUE, border='gray', col='gray80', lwd=1)
			
			if (!is.null(state)) plot(stateSpEa, col=alpha('#d95f02', 0.1), add=TRUE)
			if (!is.null(county)) plot(countySpEa, col=alpha('#d95f02', 0.1), add=TRUE)
			if (!is.null(inaccs)) plot(inaccsSpEa, col=alpha('#d95f02', 0.1), add=TRUE)
			
			# if (exists('inaccsAdminSpEa')) plot(accInaccAdminRangeSpEa, col=alpha('#d95f02', 0.1), border='#d95f02', lwd=1.8, add=TRUE)
			# plot(accRangeSpEa, col=alpha('#1b9e77', 0.25), border='#1b9e77', lwd=1.8, add=TRUE)
			
			if (!is.null(state)) plot(stateSpEa, col=alpha('#d95f02', 0.8), add=TRUE)
			if (!is.null(county)) plot(countySpEa, col=alpha('#d95f02', 0.8), add=TRUE)
			if (!is.null(inaccs)) plot(inaccsSpEa, col=alpha('#d95f02', 0.8), add=TRUE)
			points(accsSpEa, pch=21, col='black', bg=alpha('#1b9e77', 1), cex=4.5)

			legend(
				'bottomright', inset=c(0.1, 0.01),
				legend=c('Precise record', 'Imprecise record'),
				col=c('black', NA),
				lwd=c(NA, 0.5),
				fill=c(NA, alpha('#d95f02', 0.8)),
				border=c(NA, 'black'),
				pch=c(21, NA),
				pt.bg=c('#1b9e77'),
				cex=2.8,
